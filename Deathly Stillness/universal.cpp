	...
	for (int i = 0; i < ActorCount; i++)
	{
		...
		if (CurrentActor==Pawn)
		{
			...
			*((BYTE*)(Weapon + 0x2a0)) = 30; // mermi hocam
			...
			if (spread == true){					// Daðýlmama hocam
				*((float*)(Pawn + 0x688)) = 0.0f;
			}//	
			LocalPos = CurrentActorPosition;
			continue;
		}
		auto Distance = LocalPos.Distance(CurrentActorPosition) / 100.f;  //distance zaten bilmiyosan gafaný skm
		if ((Distance) >40.0f)
		{
			continue;
		}
		if (orospbu.x>ScreanX || orospbu.y>ScreanY || orospbu.y<0 || orospbu.x <0) // ekranýn içindeyse çiz yoksa çizme siktir et
		{
			continue;
		}
		auto xxa = ReadMemory<int>(CurrentActor + 0x18);
		if (xxa < 1245678 || xxa>1246010)				// entity id aralýðý
		{
			continue;
		...
		//ImGui::GetOverlayDrawList()->AddText(ImVec2(orospbu.x, orospbu.y), FLOAT4TOD3DCOLOR2(colorssssss), std::to_string(xxa).c_str());  //entity id yazdýrma için
		ImGui::GetOverlayDrawList()->AddText(ImVec2(orospbu.x, orospbu.y+10), FLOAT4TOD3DCOLOR2(colorssssss), Diste.c_str()); // buda distance yazdýrma
		if (Line)
		{
			ImGui::GetOverlayDrawList()->AddLine({ orospbu.x,(orospbu.y) }, ImVec2(io.DisplaySize.x / 2, io.DisplaySize.y / 2 * (2)), FLOAT4TOD3DCOLOR2(colorssssss), 0.7f); //esp line için bu
		}
	}
